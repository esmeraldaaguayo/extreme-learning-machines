...
def auxELMStd(folder1, hiddenNeurons, neuronType):
	# def auxELMStd(folder1, folder2, hiddenNeurons, neuronType):
	# Xts = np.load(os.path.join(folder2, "Xts_Std.npy"))
	# Tts = np.load(os.path.join(folder2, "Tts_Std.npy"))
	...
	for i in range(20):
		Xtr = np.load(os.path.join(folder1, "train_x_%d.npy" % i))
		Xts = np.load(os.path.join(folder1, "test_x_%d.npy" % i))
		Ttr = np.load(os.path.join(folder1, "train_t_%d.npy" % i))
		Tts = np.load(os.path.join(folder1, "test_t_%d.npy" % i))
		...
		# model.save("ELMmodelWeighted_1000N_Std_%d.h5" % i)
	...
	df_metrics.to_csv("Std_CV_metrics.csv", index=False)
