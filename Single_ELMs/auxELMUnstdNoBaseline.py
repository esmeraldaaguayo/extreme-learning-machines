...
def auxELMUnstdNoBaseline(folder1, hiddenNeurons, neuronType):
	# def auxELMUnstdNoBaseline(folder1, folder2, hiddenNeurons, neuronType):
	# Xts = np.load(os.path.join(folder2, "Xts_noBaselineUnstd.npy"))
	# Tts = np.load(os.path.join(folder2, "Tts_noBaselineUnstd.npy"))
	...
	for i in range(20):
		Xtr = np.load(os.path.join(folder1, "noBaselineUnstd_train_x_%d.npy" % i))
		Xts =np.load(os.path.join(folder1, "noBaselineUnstd_test_x_%d.npy" % i))
		Ttr = np.load(os.path.join(folder1, "noBaselineUnstd_train_t_%d.npy" % i))
		Tts =np.load(os.path.join(folder1, "noBaselineUnstd_test_t_%d.npy" % i))
		...
		# model.save("ELMmodelWeighted_1000N_noBaselineUnstd_%d.h5" % i)
	...
	df_metrics.to_csv("noBaselineUnstd_CV_metrics.csv", index=False)
