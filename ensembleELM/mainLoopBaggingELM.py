...
for i in range(20):
	...
	YtsList, actual, trainingTime = baggingELM(Xtr, Xts, Ttr, Tts, models)

	trainingTimeList.append(trainingTime)

	metrics, votedacc, votedrecal, votedprec = majorityVotingBagging(YtsList, actual,loop)
...
# save metrics
votedmetrics.to_csv("baggingELM_voted_metrics_{}_model.csv".format(models), index=False)
